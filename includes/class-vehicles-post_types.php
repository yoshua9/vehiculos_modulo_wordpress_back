...
class Vehicles_Post_Types {
    ...
    private function register_single_post_type( $fields ) {
        ...
        $labels = array(
            ...
            'new_item'              => sprintf( __( 'New %s', 'vehicles' ), $fields['singular'] ),
            'add_new_item'          => sprintf( __( 'Add new %s', 'vehicles' ), $fields['singular'] ),
            'edit_item'             => sprintf( __( 'Edit %s', 'vehicles' ), $fields['singular'] ),
            'view_item'             => sprintf( __( 'View %s', 'vehicles' ), $fields['singular'] ),
            'view_items'            => sprintf( __( 'View %s', 'vehicles' ), $fields['plural'] ),
            'search_items'          => sprintf( __( 'Search %s', 'vehicles' ), $fields['plural'] ),
            'not_found'             => sprintf( __( 'No %s found', 'vehicles' ), strtolower( $fields['plural'] ) ),
            'not_found_in_trash'    => sprintf( __( 'No %s found in trash', 'vehicles' ), strtolower( $fields['plural'] ) ),
            'all_items'             => sprintf( __( 'All %s', 'vehicles' ), $fields['plural'] ),
            'archives'              => sprintf( __( '%s Archives', 'vehicles' ), $fields['singular'] ),
            'attributes'            => sprintf( __( '%s Attributes', 'vehicles' ), $fields['singular'] ),
            'insert_into_item'      => sprintf( __( 'Insert into %s', 'vehicles' ), strtolower( $fields['singular'] ) ),
            'uploaded_to_this_item' => sprintf( __( 'Uploaded to this %s', 'vehicles' ), strtolower( $fields['singular'] ) ),

            /* Labels for hierarchical post types only. */
            'parent_item'           => sprintf( __( 'Parent %s', 'vehicles' ), $fields['singular'] ),
            'parent_item_colon'     => sprintf( __( 'Parent %s:', 'vehicles' ), $fields['singular'] ),
        ...
        if ( isset($fields['custom_caps']) && $fields['custom_caps'] ) {
    ...
    private function register_single_post_type_taxnonomy( $tax_fields ) {
        ...
        $labels = array(
            ...
            'all_items'                  => sprintf( __( 'All %s' , 'vehicles' ), $tax_fields['plural'] ),
            'edit_item'                  => sprintf( __( 'Edit %s' , 'vehicles' ), $tax_fields['single'] ),
            'view_item'                  => sprintf( __( 'View %s' , 'vehicles' ), $tax_fields['single'] ),
            'update_item'                => sprintf( __( 'Update %s' , 'vehicles' ), $tax_fields['single'] ),
            'add_new_item'               => sprintf( __( 'Add New %s' , 'vehicles' ), $tax_fields['single'] ),
            'new_item_name'              => sprintf( __( 'New %s Name' , 'vehicles' ), $tax_fields['single'] ),
            'parent_item'                => sprintf( __( 'Parent %s' , 'vehicles' ), $tax_fields['single'] ),
            'parent_item_colon'          => sprintf( __( 'Parent %s:' , 'vehicles' ), $tax_fields['single'] ),
            'search_items'               => sprintf( __( 'Search %s' , 'vehicles' ), $tax_fields['plural'] ),
            'popular_items'              => sprintf( __( 'Popular %s' , 'vehicles' ), $tax_fields['plural'] ),
            'separate_items_with_commas' => sprintf( __( 'Separate %s with commas' , 'vehicles' ), $tax_fields['plural'] ),
            'add_or_remove_items'        => sprintf( __( 'Add or remove %s' , 'vehicles' ), $tax_fields['plural'] ),
            'choose_from_most_used'      => sprintf( __( 'Choose from the most used %s' , 'vehicles' ), $tax_fields['plural'] ),
            'not_found'                  => sprintf( __( 'No %s found' , 'vehicles' ), $tax_fields['plural'] ),
        );
    ...
    public function create_custom_post_type() {
        ...
        $post_types_fields = array(
            array (
                ...
                'plural'                => 'Vehículos',
                'has_archive'           => true,
                'rewrite'               => array( 'slug' => 'vehiculos', "with_front" => false ),
                'hierarchical'          => false,
                ...
                'taxonomies'            => array(
                    ...
                    array(
                        'taxonomy'      => 'color_exterior',
                        'plural'        => 'Colores exteriores',
